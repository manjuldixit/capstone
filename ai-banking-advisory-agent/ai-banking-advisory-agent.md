...
## Required Method: Prompt Comparison Rule
The prototype uses `backend/run_phase3_llm_prompts.py` with three prompt variants:
...
The prompt comparison was generated by `backend/run_phase3_llm_prompts.py --provider=mock`. No real LLM credentials were available in this environment, so the mock harness returned a deterministic baseline response while preserving prompt intent differences in the generated prompt context.
...
### Key artifacts
- `backend/run_phase3_llm_prompts.py` — prompt comparison harness
...
1. Execute `backend/run_phase3_llm_prompts.py --provider=mock` to generate comparison outputs.
